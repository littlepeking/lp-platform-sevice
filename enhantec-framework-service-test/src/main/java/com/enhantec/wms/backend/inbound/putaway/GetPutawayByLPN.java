...
//			}
			...
			if ("Y".equals(isMultizones)) {
				...
				//if (!isProjectSku)
				{
					...
					ServiceDataMap res = (ServiceDataMap) ServiceHelper.executeService(context, "RFPutawayMultiZones",
				...
				ServiceDataMap res = (ServiceDataMap) ServiceHelper.executeService(context, "RFPutawayP1S1Wrapper",
						new ServiceDataHolder(new ServiceDataMap(new HashMap<String, Object>() {{
							put("storerkey", storerKey);
							put("sku", sku);
							put("lot", lot);
