...
//    private static final ILogger SCE_LOGGER = SCELoggerFactory.getInstance(PrintUtil.class);
    ...
    public static List<HashMap<String,String>> getData(Context context, Connection qqConnection, String labelName, HashMap<String,String> sqlParams)  {
...
//        SCE_LOGGER.info(" LabelName: {}", labelName);
        ...
        if (labelName != null) {
          ...
          String dataSourceStr = String.valueOf(DBHelper.getValue(context,qqConnection," SELECT DataSource FROM LabelConfig WHERE LabelName = ? ", new Object[]{labelName},"标签配置"+labelName));
...
//          tmpText.concat(dataSource.copyRange(dataSource.getOffset() + 1));
          ...
          try {
              labelData = DBHelper.executeQuery(context, qqConnection, tmpText, params.toArray(new Object[]{}));
          }catch (Exception e){
    ...
    public static void printLabel(Context context, Connection conn, String printer, String labelTempName, String labelName, String copies, String notes, List<HashMap<String,String>> data, String key) throws Exception {
        ...
        //int TaskID=XtSql.GetSeq(context, conn, "SEQ_PRINT_TASK");
        ...
        try {
            for (int i1 = 0; i1 < data.size(); i1++) {
                ...
                for(int i=0;i<copyNum;i++) {
                    LegacyDBHelper.ExecSqlByObject(context, conn, "Insert Into PRINT_TASK(AddWho,EditWho"
