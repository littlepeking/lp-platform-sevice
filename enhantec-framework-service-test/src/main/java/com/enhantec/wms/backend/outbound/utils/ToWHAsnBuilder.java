...
public class ToWHAsnBuilder {
    ...
    public ToWHAsnBuilder(Context context, String toWareHouseId, String receiptType){
        this.context = context;
        this.connection = connection;
        this.toWareHouseId = toWareHouseId.toLowerCase();
        this.userId = context.getUserID();
        this.storerKey = CDSysSet.getStorerKey(context);
    ...
    public String buildReceiptDetailInfo(String receiptKey,String sku,int seq,Map<String,String> fromIdnotesMap) throws Exception {
        ...
        LegacyDBHelper.ExecInsert(context, toWareHouseId+".RECEIPTDETAIL", receiptDetail);
    ...
    {
        HashMap<String,String> lotRuleCodelkup = DBHelper.getRecord(context,
        ...
        String skuTypeCode= DBHelper.getValue(context, "select udf4 from "+toWareHouseId+".codelkup a,sku s where a.listname=? and a.code=s.busr4 and s.sku=?", new String[]{"SKUTYPE1",sku}, "");//根据sku获批号是sm||sc
    ...
    public String buildReceiptHeadInfo(String isConfirmedUser1,String isConfirmedUser2) throws Exception {
        ...
        LegacyDBHelper.ExecInsert(context, toWareHouseId + ".RECEIPT", receipt);
        return receiptKey;
    }

    private String getKeyCount(String keyName,int length){
        String result;
        result = DBHelper.getValue(context, "SELECT KEYCOUNT FROM " + toWareHouseId + ".NCOUNTER WHERE KEYNAME = ?",
    ...
    private String buildReceiptLotXIdHeaderInfo(String sku,  String id, String sourceKey, String sourceLineNumber) throws Exception {
        ...
        LegacyDBHelper.ExecInsert(context, toWareHouseId+".LOTXIDHEADER", lotxIdHeaderHashMap);
        return lotxidKey;
    }
    ...
    private void buildReceiptLotxIdDetailInfo(String  lotxidKey, String sku, String id, String sourceKey, String sourceLineNumber, String sn,String snWgt,String snUom, int seq) throws Exception {
        HashMap<String, String> lotxIdDetail = DBHelper.getRecord(context,
        ...
        Integer count = DBHelper.getValue(context, "SELECT count(1) FROM "+toWareHouseId+".SERIALINVENTORY WHERE SKU = ? and SERIALNUMBERLONG = ? ",
        ...
        LegacyDBHelper.ExecInsert(context, toWareHouseId+".LOTXIDDETAIL", lotxIdDetailHashMap);
    }
