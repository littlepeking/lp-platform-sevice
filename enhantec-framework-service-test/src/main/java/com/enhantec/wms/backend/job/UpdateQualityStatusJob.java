...
public class UpdateQualityStatusJob extends LegacyBaseService {
	...
	public void execute(ServiceDataHolder serviceDataHolder)
	{
		...
		try
		{
			...
			List<HashMap<String,String>> list =  DBHelper.executeQuery(context, conn, "SELECT STORERKEY, SKU, ELOT, ELOTTABLE13 FROM enterprise.ELOTATTRIBUTE WHERE (ELOTTABLE03 = 'RELEASE' OR ELOTTABLE03 = 'CONDIREL') AND ELOTTABLE05 < DATEADD(s, 1, DATEADD(DAY, DATEDIFF(DAY, 0, GETDATE()), 0)) "
					, new Object[]{});
			...
			if(list.size()>0) {
				for (HashMap<String,String> tempLotAttr : list) {
					...
					String newQAStatus  = CDSysSet.getElot05ExpiredQualityStatus(context,conn);

//					if(tempLotAttr.get("ELOTTABLE13").equals("0")||
//						tempLotAttr.get("ELOTTABLE13").equals("1")) newQAStatus = "QUARANTINE";
					DBHelper.executeUpdate(context, conn, "UPDATE enterprise.ELOTATTRIBUTE SET EDITWHO=?, EDITDATE=?, ELOTTABLE03=? WHERE ELOT = ? "
							, new Object[]{userid, UtilHelper.getCurrentSqlDate(), newQAStatus, tempLotAttr.get("ELOT")});

//					//更新标签表对应的质量状态
//					DBHelper.executeUpdate(context, conn, "UPDATE IDNOTES SET EDITWHO=?, EDITDATE=?, ELOTTABLE03=? WHERE ELOT=? "
...
//
			...
			//查询过有效期的库存批次质量状态
			//SELECT DATEADD(s, 1, DATEADD(DAY, DATEDIFF(DAY, 0, GETDATE()), 0))     --first second of today
			list =  DBHelper.executeQuery(context, conn, "SELECT SKU, ELOT, ELOTTABLE13 FROM  enterprise.ELOTATTRIBUTE WHERE (ELOTTABLE03 = 'RELEASE' OR ELOTTABLE03 = 'CONDIREL' OR ELOTTABLE03 = 'QUARANTINE') AND  ELOTTABLE11 < DATEADD(s, 1, DATEADD(DAY, DATEDIFF(DAY, 7, GETDATE()), 0)) "
					, new Object[]{});
			...
			if(list.size()>0) {
				for (HashMap<String,String> tempLotAttr : list) {
					...
					String newQAStatus  = CDSysSet.getElot11ExpiredQualityStatus(context,conn);
