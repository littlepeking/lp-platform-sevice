...
public class PickDetail {
    ...
    public static HashMap<String, String> findByPickDetailKey(Context context, String id, boolean checkExist) {

        String SQL="select * from pickdetail where pickdetailkey = ?";

        HashMap<String,String> record= DBHelper.getRecord(context, SQL, new Object[]{ id},"拣货明细");
        if(checkExist && record == null) ExceptionHelper.throwRfFulfillLogicException("未找到拣货明细"+id);
        return record;
    }

    public static HashMap<String, String> findPickedLpn(Context context, String lpn, boolean checkExist) {

        String SQL="select * from pickdetail where status = 5 and id = ?";

        HashMap<String,String> record= DBHelper.getRecord(context, SQL, new Object[]{ lpn},"拣货明细");
    ...
    public static List<HashMap<String, String>> findByOrderKey(Context context, String orderkey, boolean checkExist) {
        ...
        List<HashMap<String,String>> records= DBHelper.executeQuery(context, SQL, new Object[]{ orderkey});
    ...
    public static List<HashMap<String,String>> findByOrderKeyAndOrderLineNumber(Context context,String orderKey,String orderLineNumber,boolean checkExist){
        String sql = "SELECT * FROM PICKDETAIL WHERE ORDERKEY = ? AND ORDERLINENUMBER = ?";
        List<HashMap<String, String>> records = DBHelper.executeQuery(context, sql, new Object[]{orderKey, orderLineNumber});
