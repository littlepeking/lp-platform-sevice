...
public class Itrn {
    ...
    public static  Map<String,String> findByPickDetailKey( String pickDetailKey, boolean checkExist) throws FulfillLogicException {

        if(UtilHelper.isEmpty(pickDetailKey)) ExceptionHelper.throwRfFulfillLogicException("拣货明细ID不能为空");

        Map<String,String>  itrnRecord = DBHelper.getRecord("select * from ITRN where TRANTYPE ='MV' and SOURCETYPE ='PICKING' and SOURCEKEY = ?",
