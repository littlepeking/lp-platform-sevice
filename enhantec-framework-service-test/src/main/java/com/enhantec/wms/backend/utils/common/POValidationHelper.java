...
public class POValidationHelper {
        ...
        public static void supplierCheckFromSkuSupplier(String supplier,String sku,String errmsg) throws Exception {
            supplierCheckFromSupplier(supplier,errmsg);
            int countSKUSUPPLIER = Integer.parseInt((String)DBHelper.getValue("select COUNT(1) from SKUSUPPLIER where SKU=? and SUPPLIERCODE=?",
    ...
    public static void supplierCheckFromSku(String supplier,String sku,String errmsg) throws Exception {
        int countSKUSUPPLIER = Integer.parseInt((String)DBHelper.getValue("select COUNT(1) from SKU where SKU=? and BUSR9=?",
                new Object[]{sku,supplier},errmsg));
        if(countSKUSUPPLIER==0)
            throw new Exception(errmsg);
    }
    public static void supplierCheckFromSupplier(String supplier,String errmsg) throws Exception {
        int countvendor = Integer.parseInt((String)DBHelper.getValue("select COUNT(1) from STORER where TYPE ='5' AND STORERKEY=?",
    ...
    public static void supplierValidityCheckFromSupplier(String supplier,String errmsg) throws Exception {
        ...
        String INACTIVEDATEVendor = DBHelper.getValue( "select CONVERT(varchar(100),DATEADD(hh,-11,INACTIVEDATE),120) from STORER where TYPE ='5' AND STORERKEY=?",
                new String[]{supplier},"");
        if (!"".equals(INACTIVEDATEVendor)&&!"NULL".equals(INACTIVEDATEVendor)){
            String dbdateStr = DBHelper.getValue( "SELECT CONVERT(varchar(100),getdate(),120)",new String[]{},"");
    ...
    public static void SupplierValidityCheckFromSkuSupplier(String sku,String supplier,String errmsg) throws Exception {
        ...
        String INACTIVEDATEStr = DBHelper.getValue( "select CONVERT(varchar(100),DATEADD(hh,-11,INACTIVEDATE),120) from SKUSUPPLIER where SKU=? and SUPPLIERCODE=?",
