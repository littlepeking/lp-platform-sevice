...
public class CDQualityStatus {
    ...
    public static String findByOrderType(Context context, Connection conn, String type, String sku){
        return CodeLookup.getCodeLookupByKey(context,conn,"ORDERTYPE",type).get("UDF5");
    }
     ...
     * @return
     */
    public static String findByReceiptType(Context context, Connection conn,String type,String sku,String preStatus){

            String defaultReceiptQualityStatus = CodeLookup.getCodeLookupByKey(context,conn,"RECEIPTYPE",type).get("UDF7");

            if (UtilHelper.isEmpty(defaultReceiptQualityStatus)){

                String skuIsQualityCheck = SKU.findById(context,conn,sku,true).get("BUSR3");

                return "1".equals(skuIsQualityCheck) ? Const.QUALITYSTATUS_QUARANTINE :  Const.QUALITYSTATUS_RELEASE;

            }else{

                if(CDReceiptType.isReturnType(context,conn,type) && defaultReceiptQualityStatus.equals("SAMEASBEFORE")) {
