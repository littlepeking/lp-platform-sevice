...
public class Pack {
    ...
    public static  HashMap<String,String> findById(Context context, Connection conn, String packKey, boolean checkExist) throws FulfillLogicException {

        if(UtilHelper.isEmpty(packKey)) ExceptionHelper.throwRfFulfillLogicException("包装代码不能为空");

        HashMap<String,String>  packRecord = DBHelper.getRecord(context,conn,"select * from pack where packKey = ?", new Object[]{packKey},"包装");
