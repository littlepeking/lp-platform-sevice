...
public class IDNotesHistory {
    ...
    public static Map<String, String> findLastShippedRecordById( String lpn, boolean checkExist) {
    ...
    public static List<Map<String,String>> findLastShippedSNsById( String lpn, boolean checkExist) throws FulfillLogicException {

        if(UtilHelper.isEmpty(lpn)) ExceptionHelper.throwRfFulfillLogicException("箱号不能为空");
        Map<String,String>  idLastHisRecord = IDNotesHistory.findLastShippedRecordById(lpn, true);
        List<Map<String,String>>  snList = DBHelper.executeQuery("SELECT * FROM SNHISTORY where IDSERIALKEY = ? ", new Object[]{idLastHisRecord.get("SERIALKEY")});
