...
public class Receipt {
     ...
     */
    public static void updateReceiptStatus(String receiptKey, String status){
        String sql = "UPDATE RECEIPT SET STATUS = '"+status+"' WHERE RECEIPTKEY = ?";
        DBHelper.executeUpdate(sql,new Object[]{receiptKey});
    }
     ...
     */
    public static void updateReceiptDetailStatus(String receiptKey,String receiptLineNumber,String status){
        String sql = "UPDATE RECEIPTDETAIL SET STATUS = '"+status+"' WHERE RECEIPTKEY = ? AND RECEIPTLINENUMBER = ?";
        DBHelper.executeUpdate(sql,new Object[]{receiptKey,receiptLineNumber});
    }
    ...
    public static Map<String,String> insertReceiptDetailByReturnLpn(
        ...
        if(!UtilHelper.isEmpty(originLpn) &&
                (!IDNotes.isBoxId(originLpn) && !IDNotes.isLpn(originLpn)))
            ExceptionHelper.throwRfFulfillLogicException("待收货的箱号不符合唯一码管理物料的箱号规则");
        /**
         * 唯一码收货绑定新箱号(自动输入都可)
         */
        boolean needGenerateBoxId = false;
        if(SKU.isSerialControl(sku) && CDReceiptType.isBindingNewLpn(sku,receiptHashMap.get("TYPE"))) {
            ...
            if(!UtilHelper.isEmpty(originLpn)){
                ...
                if(UtilHelper.isEmpty(newLpn)) {
                    if(CDReceiptType.isBindAndAutoGenerateLpn(sku,receiptHashMap.get("TYPE"))) {
                ...
                lpnHisInfo = SNHistory.findBySkuAndSN(sku,snList[0],true);
                lpnHisInfo.put("ISOPENED","1");//根据唯一码进行收货，说明箱子已经拆封

            }
            //整箱退货
        }else if(SKU.isSerialControl(sku) && !CDReceiptType.isBindingNewLpn(sku,receiptHashMap.get("TYPE"))) {
            ...
            if(!UtilHelper.isEmpty(originLpn)) {
                ...
                boolean useExistInventory = isAllowReceivingToExistingInv( receiptHashMap, sku, originLpn);

                if(useExistInventory){
                    //使用LPN所在库位
                    lpnHisInfo = LotxLocxId.findById(originLpn,true);
        ...
        newReceiptDetail.put("PROJECTCODEBYORDERKEY",getProjectCodeByOrderKey(lpnHisInfo.get("ORDERKEY")));//原出库项目号）
//        newReceiptDetail.put("LASTLOC", lpnHisInfo.get("LOC")); //该ID的上一个库位
        ...
        LegacyDBHelper.ExecInsert( "RECEIPTDETAIL", newReceiptDetail);

        buildReceiptLotxIdInfo( newReceiptDetail.get("SKU"),"I",newReceiptDetail.get("TOID"),newReceiptDetail.get("RECEIPTKEY"),newReceiptDetail.get("RECEIPTLINENUMBER"), snList, snWightList, snUomList);
...
//        }
    ...
    private static Map<String,String> buildReturnLotInfo( Map<String,String> receiptHashMap, Map<String,String> lpnHisInfo) throws Exception {
       ...
       if(receiptLottableInfoHashMap != null){
           ...
           String returnLotGenerateType = CDReceiptType.getReturnLotGenerateType( receiptHashMap.get("TYPE"));
           ...
           if(UtilHelper.isEmpty(returnLotGenerateType) || "0".equals(returnLotGenerateType)) {
               ...
               String newRetLot = IdGenerationHelper.createSubReceiptLot( lpnHisInfo.get("LOTTABLE06"), "R");
               ...
               String newRetLot = IdGenerationHelper.createReceiptLot( lpnHisInfo.get("SKU"));
               //Elottable07生产批号+R(A-Z)
               String newELottable07 = IdGenerationHelper.createSubReceiptLot( lpnHisInfo.get("ELOTTABLE07"), "R");
               newReceiptLotHashMap.put("LOTTABLE06", newRetLot);
...
//            receiptLot = IdGenerationHelper.createReceiptLot( originalReceiptLineHashMap.get("SKU"));
...
//                            });
        ...
        if(UtilHelper.isEmpty(lpn)){
            if(SKU.isSerialControl(originalReceiptLineHashMap.get("SKU"))) {
                lpn = IdGenerationHelper.createBoxId(receiptLot);
            }else {
                 lpn = IdGenerationHelper.generateLpn( receiptLot);
            }

        }else {
            if(SKU.isSerialControl(originalReceiptLineHashMap.get("SKU"))) {
                if (!IDNotes.isBoxId( lpn))
                    ExceptionHelper.throwRfFulfillLogicException("绑定的条码不符合唯一码管理物料的箱号规则");
            }else {
                if (!IDNotes.isLpn( lpn))
                    ExceptionHelper.throwRfFulfillLogicException("绑定的条码不符合按批次管理的容器条码规则");
            }
        }
        ...
        DBHelper.executeUpdate( "UPDATE RECEIPTDETAIL SET TOTALBARRELNUMBER = ? WHERE RECEIPTKEY = ? ", new Object[]{
        ...
        buildReceiptLotxIdInfo( newReceiptDetail.get("SKU"),"I",newReceiptDetail.get("TOID"),newReceiptDetail.get("RECEIPTKEY"),newReceiptDetail.get("RECEIPTLINENUMBER"), snList);
    ...
    public static void deleteReceiptDetail(String receiptKey,String receiptLineNumber)throws Exception{
        ...
        DBHelper.executeUpdate("DELETE FROM RECEIPTDETAIL WHERE RECEIPTKEY = ? AND RECEIPTLINENUMBER = ? ",
        new Object[]{receiptDetailInfo.get("RECEIPTKEY"), receiptDetailInfo.get("RECEIPTLINENUMBER")});

        if(SKU.isSerialControl(receiptDetailInfo.get("SKU"))) {
            DBHelper.executeUpdate( "DELETE FROM LOTXIDHEADER WHERE SOURCEKEY = ? AND SOURCELINENUMBER = ? AND IOFLAG ='I'",
                    new Object[]{receiptDetailInfo.get("RECEIPTKEY"), receiptDetailInfo.get("RECEIPTLINENUMBER")});

            DBHelper.executeUpdate( "DELETE FROM LOTXIDDETAIL WHERE SOURCEKEY = ? AND SOURCELINENUMBER = ? AND IOFLAG ='I'",
                    new Object[]{receiptDetailInfo.get("RECEIPTKEY"), receiptDetailInfo.get("RECEIPTLINENUMBER")});
        }
    ...
    private static void refreshReceiptBarrelNumber( String receiptKey,String originalLineNumber){
        ...
        if(receiptDetails!=null && receiptDetails.size()>0){
            ...
            for(int i =0;i<receiptDetails.size();i++){
                ...
                //更新所有收货明细的总桶数
                DBHelper.executeUpdate("UPDATE RECEIPTDETAIL SET TOTALBARRELNUMBER = ?, BARRELNUMBER=? WHERE RECEIPTKEY = ? AND  RECEIPTLINENUMBER = ? ",
                  new Object[]{
...
//        serviceDataHolder.getInputDataAsMap().setAttribValue("LOC", insertedReceiptDetail.get("TOLOC"));
        ...
        if(Const.RECEIPT_RF_TYPE_WITH_ASN.equalsIgnoreCase(receiptTypeHashMap.get("UDF5"))) {
            ...
            //更新所有已收货行的总桶数
            DBHelper.executeUpdate( "UPDATE IDNOTES SET TOTALBARREL = ?, BARRELDESCR = BARRELNUMBER +' / '+ ? WHERE ORIGINRECEIPTKEY = ? ", new Object[]{
    ...
    public static void processReceiptStatus(String receiptKey){
        ...
        for (Map<String, String> record : records) {
            ...
            if(UtilHelper.decimalStrCompare(qtyExpected,record.get("QTYRECEIVED")) <= 0 && !record.get("STATUS").equals("0")){
                ...
                Receipt.updateReceiptDetailStatus(receiptKey,record.get("ORIGINALLINENUMBER"),"5");
            }
        }
        ...
        Receipt.updateReceiptStatus(receiptKey,status);
