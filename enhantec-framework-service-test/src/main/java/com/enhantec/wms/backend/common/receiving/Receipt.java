...
public class Receipt {
    ...
    public static HashMap<String, String> findByReceiptKey(Context context, Connection conn, String receiptKey, boolean checkExist) {

        String SQL="SELECT * FROM RECEIPT WHERE  receiptKey = ?  ";
        HashMap<String, String>  record = DBHelper.getRecord(context, conn, SQL, new Object[]{ receiptKey},"收货单");
        if(checkExist && record == null ) ExceptionHelper.throwRfFulfillLogicException("收货单为"+receiptKey+"未找到");
        return record;
    }

    public static HashMap<String, String> findReceiptDetailByLPN(Context context, Connection conn,String receiptKey, String lpn, boolean checkExist) {

        String SQL="SELECT * FROM RECEIPTDETAIL WHERE RECEIPTKEY = ?  AND TOID = ?";
        HashMap<String,String> record= DBHelper.getRecord(context, conn, SQL, new Object[]{receiptKey, lpn},"收货明细");
     ...
     * 查询最后被收货的LPN的收货单信息
     */
    public static HashMap<String,String> findLastReceiptDetailByLPN(Context context,Connection connection,String lpn,boolean checkExist){
        String sql = "SELECT TOP 1 * FROM RECEIPTDETAIL WHERE TOID = ? ORDER BY EDITDATE DESC";
        HashMap<String, String> record = DBHelper.getRecord(context, connection, sql, new Object[]{lpn}, "收货明细");
        if(checkExist && record == null) ExceptionHelper.throwRfFulfillLogicException("容器条码为"+lpn+"的收货明细未找到");
        return record;
    }


    public static HashMap<String, String> findReceiptDetailById(Context context, Connection conn, String receiptKey, String receiptLineNumber, boolean checkExist) {

        String SQL="SELECT * FROM RECEIPTDETAIL WHERE  receiptKey = ? and receiptLineNumber = ? ";
        HashMap<String,String> record= DBHelper.getRecord(context, conn, SQL, new Object[]{ receiptKey,receiptLineNumber},"收货明细");
        if(checkExist && record == null) ExceptionHelper.throwRfFulfillLogicException("收货单为"+receiptKey+receiptLineNumber+"的收货明细未找到");
        return record;
    }

    public static List<HashMap<String, String>> findReceiptDetails(Context context, Connection conn, String receiptKey, boolean checkExist) {

        String SQL="SELECT * FROM RECEIPTDETAIL WHERE  receiptKey = ?  ";
        List<HashMap<String, String>>  list = DBHelper.executeQuery(context, conn, SQL, new Object[]{ receiptKey});
     ...
     */
    public static void updateReceiptStatus(Context context,Connection connection,String receiptKey, String status){
        String sql = "UPDATE RECEIPT SET STATUS = '"+status+"' WHERE RECEIPTKEY = ?";
        DBHelper.executeUpdate(context,connection,sql,new Object[]{receiptKey});
    }
     ...
     */
    public static void updateReceiptDetailStatus(Context context,Connection connection,String receiptKey,String receiptLineNumber,String status){
        String sql = "UPDATE RECEIPTDETAIL SET STATUS = '"+status+"' WHERE RECEIPTKEY = ? AND RECEIPTLINENUMBER = ?";
        DBHelper.executeUpdate(context,connection,sql,new Object[]{receiptKey,receiptLineNumber});
    }
     ...
     * @throws Exception
     */
    public static LinkedHashMap<String, String> insertReceiptDetailByReturnLpn(
            Context context, Connection connection, HashMap<String,String> receiptHashMap, String receiptLinuNumber,
        ...
        if(!UtilHelper.isEmpty(newLpn)) {
            ...
            HashMap<String, String> receiptDetailInfo = DBHelper.getRecord(context, connection,
            ...
            HashMap<String, String> lpnInfo = LotxLocxId.findById(context, connection, newLpn, false);
            if (lpnInfo != null) ExceptionHelper.throwRfFulfillLogicException("当前容器条码仍有库存，不允许退货");


            if (SKU.isSerialControl(context, connection, sku)) {
                if (!IDNotes.isBoxId(context, connection, newLpn))
                    ExceptionHelper.throwRfFulfillLogicException("绑定的条码不符合唯一码管理物料的箱号规则");
            } else {
                if (!IDNotes.isLpn(context, connection, newLpn))
        ...
        if(!UtilHelper.isEmpty(originLpn) &&
                (!IDNotes.isBoxId(context,connection,originLpn) && !IDNotes.isLpn(context,connection,originLpn)))
            ExceptionHelper.throwRfFulfillLogicException("待收货的箱号不符合唯一码管理物料的箱号规则");
        /**
         * 唯一码收货绑定新箱号(自动输入都可)
         */
        boolean needGenerateBoxId = false;
        if(SKU.isSerialControl(context,connection,sku) && CDReceiptType.isBindingNewLpn(context,connection,sku,receiptHashMap.get("TYPE"))) {
            ...
            if(!UtilHelper.isEmpty(originLpn)){
                ...
                if(UtilHelper.isEmpty(newLpn)) {
                    if(CDReceiptType.isBindAndAutoGenerateLpn(context,connection,sku,receiptHashMap.get("TYPE"))) {
                ...
                lpnHisInfo = SNHistory.findBySkuAndSN(context,connection,sku,snList[0],true);
                lpnHisInfo.put("ISOPENED","1");//根据唯一码进行收货，说明箱子已经拆封

            }
            //整箱退货
        }else if(SKU.isSerialControl(context,connection,sku) && !CDReceiptType.isBindingNewLpn(context,connection,sku,receiptHashMap.get("TYPE"))) {

            if(!UtilHelper.isEmpty(originLpn)){
                //唯一码物料整箱退库,箱号不变，但需要填充SN明细行
                lpnHisInfo = IDNotesHistory.findLastShippedRecordById(context,connection,originLpn,true);
                List<HashMap<String, String>> lastShippedSNsById = IDNotesHistory.findLastShippedSNsById(context, connection, originLpn, true);
                ...
                newLpn = IdGenerationHelper.generateIDByKeyName(context, connection,context.getUserID(), "WMSBOX",10);
            }
            ...
            if(!UtilHelper.isEmpty(originLpn)) {
                ...
                boolean useExistInventory = isAllowReceivingToExistingInv(context, connection, receiptHashMap, sku, originLpn);

                if(useExistInventory){
                    //使用LPN所在库位
                    lpnHisInfo = LotxLocxId.findById(context,connection,originLpn,true);
        ...
        HashMap<String, String> newReceiptLotHashMap = buildReturnLotInfo(context,connection, receiptHashMap, lpnHisInfo);
        //
        if(needGenerateBoxId){
            newLpn = IdGenerationHelper.createBoxId(context,connection,newReceiptLotHashMap.get("LOTTABLE06"));
        }
        ...
        String receiptKey = DBHelper.getValue(context, connection, receiptDetailSql, new Object[]{newLpn}, "", false);
        ...
        LinkedHashMap<String, String> newReceiptDetail = new LinkedHashMap<>();

        String storerKey = java.lang.String.valueOf(DBHelper.getValue(context, connection, "select UDF1 from Codelkup where ListName=? and Code=?",
        ...
        newReceiptDetail.put("PROJECTCODEBYORDERKEY",getProjectCodeByOrderKey(context,connection,lpnHisInfo.get("ORDERKEY")));//原出库项目号）
//        newReceiptDetail.put("LASTLOC", lpnHisInfo.get("LOC")); //该ID的上一个库位
        ...
        String thisReceiptLineNumber = DBHelper.getValue(context, connection, "SELECT CASE WHEN MAX(RECEIPTLINENUMBER) IS NULL THEN 0 ELSE MAX(RECEIPTLINENUMBER) END FROM RECEIPTDETAIL WHERE RECEIPTKEY = ?",
        ...
        LegacyDBHelper.ExecInsert(context, connection, "RECEIPTDETAIL", newReceiptDetail);

        buildReceiptLotxIdInfo(context, connection, newReceiptDetail.get("SKU"),"I",newReceiptDetail.get("TOID"),newReceiptDetail.get("RECEIPTKEY"),newReceiptDetail.get("RECEIPTLINENUMBER"), snList, snWightList, snUomList);
...
//        }
    ...
    private static boolean isAllowReceivingToExistingInv(Context context, Connection connection, HashMap<String, String> receiptHashMap, String sku, String originLpn) {

        boolean useExistInventory = false;
        if(!SKU.isSerialControl(context, connection, sku) &&CDReceiptType.isReturnTypeWithInventory(context, connection, receiptHashMap.get("TYPE")) ){

            HashMap<String, String> lotxLocxIdById = LotxLocxId.findById(context, connection, originLpn, false);
    ...
    //生成退货批属性信息
    private static HashMap<String,String> buildReturnLotInfo(Context context, Connection conn, HashMap<String,String> receiptHashMap, HashMap<String,String> lpnHisInfo) throws Exception {

       HashMap<String,String> newReceiptLotHashMap = new HashMap<>();

       //RECEIPTDETAIL.SUSR10 退货LPN的原收货批次，用于收货时供相同批次的LPN检索已生成的子批号，避免子批次重复生成。
       HashMap<String,String> receiptLottableInfoHashMap = DBHelper.getRecord(context,conn,"SELECT TOP 1 * FROM RECEIPTDETAIL WHERE RECEIPTKEY = ? AND SUSR10 = ? ",
       ...
       if(receiptLottableInfoHashMap != null){
           ...
           String returnLotGenerateType = CDReceiptType.getReturnLotGenerateType(context,conn, receiptHashMap.get("TYPE"));
           ...
           if(UtilHelper.isEmpty(returnLotGenerateType) || "0".equals(returnLotGenerateType)) {
               ...
               String newRetLot = IdGenerationHelper.createSubReceiptLot(context, conn, lpnHisInfo.get("LOTTABLE06"), "R");
               ...
               String newRetLot = IdGenerationHelper.createReceiptLot(context, conn, lpnHisInfo.get("SKU"));
               //Elottable07生产批号+R(A-Z)
               String newELottable07 = IdGenerationHelper.createSubReceiptLot(context, conn, lpnHisInfo.get("ELOTTABLE07"), "R");
               newReceiptLotHashMap.put("LOTTABLE06", newRetLot);
     ...
     * @throws Exception
     */
    public static LinkedHashMap<String, String> insertReceiptDetailByOriginalLine(Context context, Connection connection, HashMap<String,String> originalReceiptLineHashMap,String lpn,String loc, String isOpened, String netStdWgt, String grossStdWgt, String tareStdWgt, String uom, String regrossWgt,String[] snList)throws Exception {
...
//            receiptLot = IdGenerationHelper.createReceiptLot(context, connection, originalReceiptLineHashMap.get("SKU"));
//            DBHelper.executeUpdate(context,connection,"UPDATE RECEIPTDETAIL SET LOTTABLE06 = ? WHERE RECEIPTKEY = ? AND RECEIPTLINENUMBER = ?",
//                    new Object[]{
//                                    receiptLot),
...
//                            });
        ...
        if(UtilHelper.isEmpty(lpn)){
            if(SKU.isSerialControl(context,connection,originalReceiptLineHashMap.get("SKU"))) {
                lpn = IdGenerationHelper.createBoxId(context,connection,receiptLot);
            }else {
                 lpn = IdGenerationHelper.generateLpn(context,connection, receiptLot);
            }

        }else {
            if(SKU.isSerialControl(context,connection,originalReceiptLineHashMap.get("SKU"))) {
                if (!IDNotes.isBoxId(context, connection, lpn))
                    ExceptionHelper.throwRfFulfillLogicException("绑定的条码不符合唯一码管理物料的箱号规则");
            }else {
                if (!IDNotes.isLpn(context, connection, lpn))
                    ExceptionHelper.throwRfFulfillLogicException("绑定的条码不符合按批次管理的容器条码规则");
            }
        }
        ...
        String maxBarrelNumber = DBHelper.getValue(context, connection, "SELECT CASE WHEN MAX(BARRELNUMBER) IS NULL THEN 0 ELSE MAX(BARRELNUMBER) END FROM RECEIPTDETAIL WHERE ORIGINALLINENUMBER IS NOT NULL AND RECEIPTKEY = ? ",
        ...
        DBHelper.executeUpdate(context, connection, "UPDATE RECEIPTDETAIL SET TOTALBARRELNUMBER = ? WHERE RECEIPTKEY = ? ", new Object[]{
        ...
        buildReceiptLotxIdInfo(context, connection, newReceiptDetail.get("SKU"),"I",newReceiptDetail.get("TOID"),newReceiptDetail.get("RECEIPTKEY"),newReceiptDetail.get("RECEIPTLINENUMBER"), snList);
     ...
     * @throws Exception
     */
    public static void deleteReceiptDetail(Context context, Connection connection,String receiptKey,String receiptLineNumber)throws Exception{

        HashMap<String,String> receiptDetailInfo = Receipt.findReceiptDetailById(context,connection,receiptKey,receiptLineNumber,true);

        if(!receiptDetailInfo.get("STATUS").equals("0"))  ExceptionHelper.throwRfFulfillLogicException("当前收货单行已经收货，不允许删除");

        DBHelper.executeUpdate(context,connection,"DELETE FROM RECEIPTDETAIL WHERE RECEIPTKEY = ? AND RECEIPTLINENUMBER = ? ",
        new Object[]{receiptDetailInfo.get("RECEIPTKEY"), receiptDetailInfo.get("RECEIPTLINENUMBER")});

        if(SKU.isSerialControl(context,connection,receiptDetailInfo.get("SKU"))) {
            DBHelper.executeUpdate(context, connection, "DELETE FROM LOTXIDHEADER WHERE SOURCEKEY = ? AND SOURCELINENUMBER = ? AND IOFLAG ='I'",
                    new Object[]{receiptDetailInfo.get("RECEIPTKEY"), receiptDetailInfo.get("RECEIPTLINENUMBER")});

            DBHelper.executeUpdate(context, connection, "DELETE FROM LOTXIDDETAIL WHERE SOURCEKEY = ? AND SOURCELINENUMBER = ? AND IOFLAG ='I'",
                    new Object[]{receiptDetailInfo.get("RECEIPTKEY"), receiptDetailInfo.get("RECEIPTLINENUMBER")});
        }

        HashMap<String,String> receiptTypeHashMap = CodeLookup.getCodeLookupByKey(context,connection, "RECEIPTYPE", receiptDetailInfo.get("TYPE"));
        if(Const.RECEIPT_RF_TYPE_WITH_ASN.equalsIgnoreCase(receiptTypeHashMap.get("UDF5"))) {
            refreshReceiptBarrelNumber(context, connection, receiptKey, receiptDetailInfo.get("ORIGINALLINENUMBER"));
        }
    }

    private static void refreshReceiptBarrelNumber(Context context, Connection connection, String receiptKey,String originalLineNumber){

        List<HashMap<String,String>> receiptDetails = DBHelper.executeQuery(
                context,connection,"SELECT RECEIPTLINENUMBER FROM RECEIPTDETAIL WHERE RECEIPTKEY = ? AND ORIGINALLINENUMBER = ? ",
        ...
        if(receiptDetails!=null && receiptDetails.size()>0){
            ...
            for(int i =0;i<receiptDetails.size();i++){
                ...
                //更新所有收货明细的总桶数
                DBHelper.executeUpdate(context,connection,"UPDATE RECEIPTDETAIL SET TOTALBARRELNUMBER = ?, BARRELNUMBER=? WHERE RECEIPTKEY = ? AND  RECEIPTLINENUMBER = ? ",
                  new Object[]{
    ...
    public static void execReceiptDetailReceiving(Context context, Connection connection, ServiceDataHolder serviceDataHolder, LinkedHashMap<String, String> insertedReceiptDetail, String grossWgt, String tareWgt, String netWgt) throws Exception {
        //调用API执行收货。
        ...
        HashMap<String,String> receiptTypeHashMap = CodeLookup.getCodeLookupByKey(context,connection, "RECEIPTYPE", insertedReceiptDetail.get("TYPE"));

        //有汇总指令的ASN收货要更新桶号信息
        if(Const.RECEIPT_RF_TYPE_WITH_ASN.equalsIgnoreCase(receiptTypeHashMap.get("UDF5"))) {

            //更新所有已收货行的总桶数
            DBHelper.executeUpdate(context, connection, "UPDATE IDNOTES SET TOTALBARREL = ?, BARRELDESCR = BARRELNUMBER +' / '+ ? WHERE ORIGINRECEIPTKEY = ? ", new Object[]{
     ...
     * 处理收货单汇总行的状态。
     */
    public static void processReceiptStatus(Context context,Connection connection,String receiptKey){

        //如果明细收货行收了汇总收货行的货，明细收货行应显示为已接收，更新汇总收货行的状态为5（收货中）。
        List<HashMap<String, String>> records = DBHelper.executeQuery(context, connection,
        ...
        for (HashMap<String, String> record : records) {
            ...
            String qtyExpected = DBHelper.getValue(context, connection,
                    "SELECT QTYEXPECTED FROM RECEIPTDETAIL WHERE RECEIPTKEY = ? AND RECEIPTLINENUMBER = ?",
            ...
            if(UtilHelper.decimalStrCompare(qtyExpected,record.get("QTYRECEIVED")) <= 0 && !record.get("STATUS").equals("0")){
                Receipt.updateReceiptDetailStatus(context,connection,receiptKey,record.get("ORIGINALLINENUMBER"),"9");
            }else{
                Receipt.updateReceiptDetailStatus(context,connection,receiptKey,record.get("ORIGINALLINENUMBER"),"5");
            }
        }

        //这里更新的是收货单单头的状态。
        HashMap<String, String> record = DBHelper.getRecord(context, connection,
        ...
        Receipt.updateReceiptStatus(context,connection,receiptKey,status);

    }

    public static String getProjectCodeByOrderKey(Context context,Connection connection,String orderKey){
        if (UtilHelper.isEmpty(orderKey))return " ";
        String projectCodeByOrderKey = DBHelper.getValue(context, connection,
                "SELECT NOTES FROM ORDERS WHERE ORDERKEY = ? ",
