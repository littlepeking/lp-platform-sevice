...
public class ConfirmSO extends LegacyBaseService {
    ...
    public void execute(ServiceDataHolder serviceDataHolder) {
        ...
        try {
            ...
            String isTransferOrderType = DBHelper.getValue(context,conn,
                    "SELECT EXT_UDF_STR5 FROM CODELKUP WHERE LISTNAME = 'ORDERTYPE' AND CODE = ?",
            ...
            if("Y".equals(isTransferOrderType)){
                checkTransferOrder(context,conn,ORDERSInfo);
            }
            ...
            if(esignatureKey.indexOf(':')==-1) {
                ...
                if (!UtilHelper.isEmpty(ORDERSInfo.get("ISCONFIRMEDUSER"))) {
                    ...
                    if (ORDERSInfo.get("ISCONFIRMEDUSER").equals(ISCONFIRMEDUSER)) {
                        ...
                        //复核操作
                        DBHelper.executeUpdate(context, conn, "UPDATE ORDERS SET ISCONFIRMEDUSER2 = ? , ISCONFIRMED = 2 WHERE ORDERKEY = ? ",
                                new Object[]{ISCONFIRMEDUSER, ORDERKEY});
                    }
                } else {
                    //确认操作
                    DBHelper.executeUpdate(context, conn, "UPDATE ORDERS SET ISCONFIRMEDUSER = ? , ISCONFIRMED = 1 WHERE ORDERKEY = ? ",
                            new Object[]{ISCONFIRMEDUSER, ORDERKEY});
                }
                ...
                //复核操作
                DBHelper.executeUpdate(context, conn, "UPDATE ORDERS SET ISCONFIRMEDUSER = ? ,ISCONFIRMEDUSER2 = ? , ISCONFIRMED = 2 WHERE ORDERKEY = ? ",
    ...
    private void checkTransferOrder(Context context, Connection connection, HashMap<String,String> orderInfo){
        boolean isTransferGmp = "Y".equalsIgnoreCase(CodeLookup.getCodeLookupByKey(context,connection,"WHTRANFER",orderInfo.get("TOWAREHOUSE")).get("UDF1"));
        if(isTransferGmp) {
            List<HashMap<String, String>> orderDetails = Orders.findOrderDetailsByOrderKey(context, connection, orderInfo.get("ORDERKEY"), false);
